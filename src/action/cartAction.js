...
const addToCart =
  ...
  async (dispatch) => {
    try {
      ...
      dispatch(
        commonUiActions.showToastMessage("카트에 상품 추가 완료", "success")
      );
    } catch (err) {
...
const updateQty = (id, size, qty) => async (dispatch) => {
  try {
    dispatch({ type: types.GET_CART_QTY_REQUEST });
    const response = await api.put("/cart", { productId: id, size, qty });
    if (response.status !== 200) throw new Error(response.error);
    dispatch({
